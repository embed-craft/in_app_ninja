...
class PIPNudgeRendererV2 extends StatefulWidget {
  ...
  @override
  State<PIPNudgeRendererV2> createState() => _PIPNudgeRendererV2State();
}

class _PIPNudgeRendererV2State extends State<PIPNudgeRendererV2> with SingleTickerProviderStateMixin {
  // Youtube Controller
  ...
  void _initPlayer() {
    ...
    // Look for videoUrl in config or in the first video component
    String? videoUrl = config['videoUrl']?.toString();
    
    if (videoUrl == null) {
      // Search in components
      final components = config['components'] as List? ?? [];
      for (var c in components) {
        if (c['type'] == 'video' && c['content']?['url'] != null) {
          videoUrl = c['content']['url'];
          break;
        }
      }
    }
      ...
      if (videoId != null) {
        _youtubeController = YoutubePlayerController(
          ...
          flags: const YoutubePlayerFlags(
            autoPlay: true,
            mute: true, // Start muted by default like Dashboard
            loop: true,
            hideControls: true,
  ...
  void _calculateInitialPosition() {
    ...
    final config = widget.campaign.config;
    final pos = config['position']?.toString() ?? 'bottom-right';
    final width = (config['width'] as num?)?.toDouble() ?? 160.0;
    final height = (config['height'] as num?)?.toDouble() ?? 220.0;
    
    // Responsive overrides
    final responsiveConfig = _resolveResponsiveConfig(config, size.width);
    final margin = (responsiveConfig['margin'] as num?)?.toDouble() ?? 16.0;
    double x, y;

    switch (pos) {
      case 'top-left':
        x = margin;
        y = margin + padding.top;
        break;
      case 'top-right':
      ...
      case 'bottom-left':
        x = margin;
        y = size.height - height - margin - padding.bottom;
        break;
      case 'bottom-right':
      default:
        x = size.width - width - margin;
  ...
  void _toggleMaximize() {
    setState(() {
      ...
      if (_isMaximized) {
        // Reset position to cover screen (handled in build)
      } else {
        // Restore position (handled in build via _position)
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final width = _isMaximized ? size.width : ((responsiveConfig['width'] as num?)?.toDouble() ?? 160.0);
    final height = _isMaximized ? size.height : ((responsiveConfig['height'] as num?)?.toDouble() ?? 220.0);
    final backgroundColor = _parseColor(responsiveConfig['backgroundColor']) ?? Colors.black;
    final cornerRadius = _isMaximized ? 0.0 : ((responsiveConfig['cornerRadius'] as num?)?.toDouble() ?? 12.0);
    final showCloseButton = responsiveConfig['showCloseButton'] != false;
    
    final margin = (responsiveConfig['margin'] as num?)?.toDouble() ?? 16.0;
    
    final left = _isMaximized ? 0.0 : (_position!.dx < margin ? margin : (_position!.dx > size.width - width - margin ? size.width - width - margin : _position!.dx));
    final top = _isMaximized ? 0.0 : (_position!.dy < margin ? margin : (_position!.dy > size.height - height - margin ? size.height - height - margin : _position!.dy));
            ...
            child: GestureDetector(
              ...
              child: Container(
                decoration: BoxDecoration(
                  ...
                  boxShadow: _isMaximized ? null : (_parseBoxShadow(responsiveConfig['boxShadow']) ?? [
                    BoxShadow(
                ...
                child: Stack(
                  children: [
                    // Content (Video + Overlays)
                    _buildContent(responsiveConfig),
                    
                    // Controls Overlay (Gradient + Buttons)
                    Positioned(
                      top: 0,
                      left: 0,
                      right: 0,
  ...
  List<BoxShadow>? _parseBoxShadow(dynamic shadows) {
  ...
  Widget _buildFlexibleLayout(Map<String, dynamic> config) {
    ...
    final children = sortedComponents.map((c) {
