...
class _FloaterNudgeRendererState extends State<FloaterNudgeRenderer> with SingleTickerProviderStateMixin {
  ...
  // Smooth drag support using ValueNotifier (avoids setState rebuilds)
  ...
  Widget build(BuildContext context) {
    final config = widget.campaign.config;
    debugPrint('InAppNinja: DEBUG DART FLOATER CONFIG: $config'); // ✅ DEBUG LOG
    ...
    final floaterWidth = NinjaLayerUtils.parseResponsiveSize(responsiveConfig['width'], context) ?? (isImageOnly ? null : 280.0);
    // ✅ FIX: Parse explicit height if provided (e.g. "60%") - Default 180 to match Dashboard
    final floaterHeight = NinjaLayerUtils.parseResponsiveSize(responsiveConfig['height'], context, isVertical: true) ?? (isImageOnly ? null : 180.0);
    
    final backgroundColor = isImageOnly ? Colors.transparent : (NinjaLayerUtils.parseColor(responsiveConfig['backgroundColor']) ?? const Color(0xFF000000)); // Black fallback to match Dashboard
    final borderRadius = isImageOnly ? 0.0 : (NinjaLayerUtils.parseDouble(responsiveConfig['borderRadius']) ?? 0.0);
    ...
    debugPrint('InAppNinja: 🎯 Floater hasAbsolutePositionedLayers=$hasAbsolutePositionedLayers');
    
    final padding = (isImageOnly || hasAbsolutePositionedLayers) 
        ? EdgeInsets.zero 
        : NinjaLayerUtils.parsePadding(responsiveConfig['padding']);
    final showCloseButton = responsiveConfig['showCloseButton'] == true; // PARITY: Dashboard requires explicit true
    ...
    final minHeight = NinjaLayerUtils.parseResponsiveSize(responsiveConfig['minHeight'], context, isVertical: true) ?? (isImageOnly ? 0.0 : (floaterHeight ?? 60.0));
    ...
    final offsetX = NinjaLayerUtils.parseDouble(responsiveConfig['offsetX'], context) ?? 20.0; // Default 20 to match Dashboard
    final offsetY = NinjaLayerUtils.parseDouble(responsiveConfig['offsetY'], context, true) ?? 20.0; // Default 20 to match Dashboard
    ...
    // PARITY FIX: Match Dashboard's default shadow: '0 8px 32px rgba(0, 0, 0, 0.15)'
    ...
    Widget floaterContent = Container(
      width: floaterWidth,
      height: floaterHeight,
      constraints: BoxConstraints(
        maxHeight: maxHeight,
        maxWidth: MediaQuery.of(context).size.width, // PARITY: 100% to match Dashboard
        minHeight: minHeight,
      ),
        ...
        children: [
          SingleChildScrollView(
            ...
            child: Padding(
              ...
              // PARITY FIX: Use actual floater height (explicit or constrained) for positioning
              child: _buildContent(responsiveConfig, 
                width: floaterWidth, 
                height: floaterHeight ?? maxHeight, // Use maxHeight if no explicit height
              ),
            ),
          ),
          if (showCloseButton)
            Positioned(
              ...
              top: NinjaLayerUtils.scaleValue(4, Size(floaterWidth ?? 280, floaterHeight ?? 180), isVertical: true) ?? 4,
              right: NinjaLayerUtils.scaleValue(4, Size(floaterWidth ?? 280, floaterHeight ?? 180)) ?? 4,
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    // PARITY: Dashboard uses rgba(0,0,0,0.05) for normal state
                    color: isImageOnly ? Colors.black.withOpacity(0.5) : Colors.black.withOpacity(0.05),
                  ),
                  // Smaller icon size
                  child: Icon(Icons.close, size: 14, color: isImageOnly ? Colors.white : const Color(0xFF6B7280)),
                ),
              ),
            ),
        ],
    ...
    floaterContent = _applyFilters(floaterContent, responsiveConfig);
    
    // PARITY FIX: Apply opacity if set (Dashboard uses floaterLayer.style?.opacity)
    final containerOpacity = (responsiveConfig['opacity'] as num?)?.toDouble() ?? 1.0;
    if (containerOpacity < 1.0) {
      floaterContent = Opacity(opacity: containerOpacity, child: floaterContent);
    }
    
    // ✅ FEATURE: Support Container Actions (e.g. click whole floater)
    if (responsiveConfig['action'] != null) {
      floaterContent = _buildInteraction(floaterContent, {
        'type': 'container',
        'content': {
    ...
    // FLOATER: Parse position (corner-based positioning)
    final position = responsiveConfig['position']?.toString() ?? 'bottom-right';
    debugPrint('InAppNinja: 🎯 Floater position: $position, offsetX: $offsetX, offsetY: $offsetY');

    // Build positioned floater based on corner position
    Widget positionedFloater;
    
    // Track drag start position for tap vs drag distinction
    Offset _dragStartOffset = Offset.zero;
    
    // Create the animated content wrapper with SMOOTH drag support
    // Using ValueListenableBuilder to avoid full widget rebuilds - only Transform updates
    Widget animatedContent = ValueListenableBuilder<Offset>(
      ...
      child: GestureDetector(
        // Allow drag from anywhere on the floater (even transparent areas)
        behavior: HitTestBehavior.opaque,
        // Enable drag gesture detection
        onPanStart: (details) {
          _isDragging = true;
          _dragStartOffset = _dragOffsetNotifier.value;
        },
        onPanUpdate: (details) {
          // Directly update ValueNotifier - no setState needed for smooth 60fps
          _dragOffsetNotifier.value = _dragOffsetNotifier.value + details.delta;
        },
        onPanEnd: (details) {
          _isDragging = false;
        },
        // TAP vs DRAG: Only fire tap if drag distance was minimal (< 10 pixels)
        onTap: () {
          // If we have a container action and didn't drag significantly, handle it
          if (responsiveConfig['action'] != null) {
            // Action is handled by _buildInteraction wrapper
          }
        },
    ...
    // Apply SafeArea and position based on config
    switch (position) {
      case 'bottom-right':
        positionedFloater = Positioned(
          bottom: offsetY,
          right: offsetX,
          child: SafeArea(child: animatedContent),
        );
        break;
      case 'bottom-left':
        positionedFloater = Positioned(
      ...
      case 'bottom-center':
        positionedFloater = Positioned(
          ...
          left: 0,
          right: 0,
          child: SafeArea(child: Center(child: animatedContent)),
        );
        break;
      case 'top-right':
        positionedFloater = Positioned(
    ...
    // Check if overlay is enabled for Floater
    final overlayEnabled = responsiveConfig['overlay']?['enabled'] == true;
    // NEW: Check if tap outside to dismiss is enabled (independent of overlay)
    final dismissOnTapOutside = responsiveConfig['dismissOnTapOutside'] == true;

    return Stack(
      children: [
        // NEW: Invisible tap layer for dismiss on tap outside (if enabled, but NO overlay)
        if (dismissOnTapOutside && !overlayEnabled)
          Positioned.fill(
            child: GestureDetector(
              onTap: _handleDismiss,
        ...
        // Optional Backdrop/Overlay for Floater (only if enabled)
        if (overlayEnabled)
          GestureDetector(
            onTap: () {
               // Overlay dismiss on click OR general tap outside enabled
               if (responsiveConfig['overlay']?['dismissOnClick'] == true || dismissOnTapOutside) {
                  _handleDismiss();
               }
            },
            child: FadeTransition(
        ...
        // Floater Content (positioned at corner)
        positionedFloater,
      ],
    );
  }
  ...
  Map<String, dynamic> _resolveResponsiveConfig(Map<String, dynamic> config, double screenWidth) {
    debugPrint('InAppNinja: Floater _resolveResponsiveConfig called');
    // Basic config
    var finalConfig = Map<String, dynamic>.from(config);
    
    // ✅ PARITY FIX: Prioritize floaterConfig over root config
    // Dashboard stores correct dimensions in floaterConfig (or fall back to modalConfig)
    final floaterConfig = config['floaterConfig'] as Map<String, dynamic>? ?? 
                          config['modalConfig'] as Map<String, dynamic>?;
    if (floaterConfig != null) {
      // Merge floaterConfig values, overriding root config
      if (floaterConfig['width'] != null) finalConfig['width'] = floaterConfig['width'];
      if (floaterConfig['height'] != null) finalConfig['height'] = floaterConfig['height'];
      if (floaterConfig['backgroundColor'] != null) finalConfig['backgroundColor'] = floaterConfig['backgroundColor'];
      if (floaterConfig['backgroundImageUrl'] != null) finalConfig['backgroundImageUrl'] = floaterConfig['backgroundImageUrl'];
      if (floaterConfig['backgroundSize'] != null) finalConfig['backgroundSize'] = floaterConfig['backgroundSize'];
      if (floaterConfig['backgroundPosition'] != null) finalConfig['backgroundPosition'] = floaterConfig['backgroundPosition'];
      if (floaterConfig['borderRadius'] != null) finalConfig['borderRadius'] = floaterConfig['borderRadius'];
      if (floaterConfig['showCloseButton'] != null) finalConfig['showCloseButton'] = floaterConfig['showCloseButton'];
      if (floaterConfig['overlay'] != null) finalConfig['overlay'] = floaterConfig['overlay'];
      if (floaterConfig['position'] != null) finalConfig['position'] = floaterConfig['position'];
      if (floaterConfig['offsetX'] != null) finalConfig['offsetX'] = floaterConfig['offsetX'];
      if (floaterConfig['offsetY'] != null) finalConfig['offsetY'] = floaterConfig['offsetY'];
      if (floaterConfig['shape'] != null) finalConfig['shape'] = floaterConfig['shape'];
      debugPrint('InAppNinja: 📐 Applied floaterConfig overrides: width=${floaterConfig['width']}, height=${floaterConfig['height']}, position=${floaterConfig['position']}');
    }
